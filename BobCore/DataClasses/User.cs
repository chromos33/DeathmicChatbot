...
namespace BobCore.DataClasses
{
    public class User
    {
        ...
        public List<DataClasses.Stream> Streams = new List<DataClasses.Stream>();
        ...
        {
            IEnumerable<DataClasses.Stream> stream = Streams.Where(x => x.name == name.ToLower());
            if (stream.Count() > 0)
            {
                return true;
            }
        ...
        {
            IEnumerable<DataClasses.Stream> stream = Streams.Where(x => x.name == streamname.ToLower());
            if (stream.Count() > 0)
            {
        ...
        public bool subscribeStream(string streamname, bool subscribe)
        {
            if (/*_password == password*/true)
            {
                IEnumerable<DataClasses.Stream> streams = Streams.Where(x => x.name.ToLower() == streamname.ToLower());
                if (streams.Count() > 0)
                {
        ...
        public bool addStream(string streamname, bool subscribe = false)
        {
            ...
            {
                Streams.Add(new DataClasses.Stream(streamname.ToLower(), subscribe));
                return true;
            }
            return false;
        }
